...
cmd:option('-decision_model_size', 128, 'decision net size')
...
if opt.model_rnn == 'lstm' then
    opt.model_rnn_states = 2 * opt.model_lower_rnn_layers
elseif opt.model_rnn == 'gru' then
    opt.model_rnn_states = opt.model_lower_rnn_layers
end
...
if opt.model_comm_narrow == 0 and opt.game_comm_bits > 0 and opt.model_dial == 0 then
...
opt.game_upper_action_space = opt.nsteps 
...
if opt.game_comm_bits > 0 and opt.game_nagents > 1 then
    ...
    opt.game_lower_action_space_total = opt.game_lower_action_space + opt.game_comm_bits
else
    opt.game_lower_action_space_total = opt.game_lower_action_space
end

-- Initialise models
local upperModel = (require('model.' .. 'DecisionPlan'))(opt)
local lowerModel = (require('model.' .. 'ComplexLowerPlan'))(opt)
...
--print('run episode '.. e) 
    ...
    if opt.game_comm_bits > 0 and opt.game_nagents > 1 then
        lower_episode[step].comm = torch.zeros(opt.bs, opt.game_nagents, opt.game_comm_bits):type(opt.dtype)
        if opt.model_dial == 1 and opt.model_target == 1 then
            lower_episode[step].comm_target = lower_episode[step].comm:clone()
        end
        lower_episode[step].d_comm = torch.zeros(opt.bs, opt.game_nagents, opt.game_comm_bits):type(opt.dtype)
    end
        ...
        for i = 1, opt.game_nagents do
            ...
            -- Compute Q values
            local q_t
            q_t = upperModel.agent[upperModel.id(step, i)]:forward(agent[i].upper_input[step])
    ...
    time_target = upper_episode[1].a_t:clone()
    ...
    while step <= steps and lower_episode.ended:sum() < opt.bs do
        ...
        if opt.game_comm_bits > 0 and opt.game_nagents > 1 then
            lower_episode[step + 1].comm = torch.zeros(opt.bs, opt.game_nagents, opt.game_comm_bits):type(opt.dtype)
            lower_episode[step + 1].d_comm = torch.zeros(opt.bs, opt.game_nagents, opt.game_comm_bits):type(opt.dtype)
            if opt.model_dial == 1 and opt.model_target == 1 then
                lower_episode[step + 1].comm_target = torch.zeros(opt.bs, opt.game_nagents, opt.game_comm_bits):type(opt.dtype)
            end
        end
        ...
        if opt.model_dial == 0 then
            lower_episode[step].a_comm_t = torch.zeros(opt.bs, opt.game_nagents):type(opt.dtype)
        end
        ...
        for i = 1, opt.game_nagents do
            ...
            if opt.game_comm_bits > 0 and opt.game_nagents > 1 then
                local comm = lower_episode[step].comm:clone():type(opt.dtype)
		    ...
		    if i == 1 then
                ...
                table.insert(agent[i].lower_input[step], comm_lim)
		...
		if test_mode then
            ...
            action_range = game:getLowerActionRange(step, i, time_target)
            ...
            if action_range then
                ...
                for b = 1, opt.bs do
                    ...
                    if opt.model_dial == 0 then
                        ...
                        if action_range_comm[b][2][1] > 0 then
                            ...
                            max_a_comm[b] = agent[i].lower_range[{ action_range_comm[b][2] }][a:squeeze()]
                        end
                    else
                        local v, a = torch.max(q_t[action_range[b]], 2)
                        max_a[b] = agent[i].lower_range[{ action_range[b][2] }][a:squeeze()]
                ...
                if opt.model_dial == 0 and opt.game_comm_bits > 0 then
                    _, max_a = torch.max(q_t[{ {}, { 1, opt.game_lower_action_space } }], 2)
                    _, max_a_comm = torch.max(q_t[{ {}, { opt.game_lower_action_space + 1, opt.game_lower_action_space_total } }], 2)
                    max_a_comm = max_a_comm + opt.game_lower_action_space
                else
                    _, max_a = torch.max(q_t, 2)
                end
            ...
            if opt.model_dial == 0 and opt.game_comm_bits > 0 then
                lower_episode[step].a_comm_t[{ {}, { i } }] = max_a_comm:type(opt.dtype)
            end
		     ...
		     if opt.model_dial == 0 then
                        ...
                        if torch.uniform() < opt.lower_eps then
                            ...
                            if action_range then
                                ...
                                lower_episode[step].a_t[b][i] = torch.random(opt.game_lower_action_space)
                            end
                        end

                        -- Random communication
                        if opt.game_comm_bits > 0 and torch.uniform() < opt.lower_eps then
                            if action_range then
                                if action_range_comm[b][2][1] > 0 then
                                    local a_range = agent[i].lower_range[{ action_range_comm[b][2] }]
                                    local a_idx = torch.random(a_range:nElement())
                                    lower_episode[step].a_comm_t[b][i] = agent[i].lower_range[{ action_range_comm[b][2] }][a_idx]
                                end
                            else
                                lower_episode[step].a_comm_t[b][i] = torch.random(opt.game_lower_action_space + 1, opt.game_lower_action_space_total)
                            end
                        end
		...
		-- use adviced action for imitation learning
                ...
                if step <= opt.nsteps then
                    ...
                    if opt.model_dial == 1 then
                        lower_episode[step + 1].comm[b][i] = comm[b]
                    else
                        local a_t = lower_episode[step].a_comm_t[b][i] - opt.game_lower_action_space
                        if a_t > 0 then
                            lower_episode[step + 1].comm[b][{ { i }, { a_t } }] = 1
                        end
                    end
        ...
        lower_episode[step].r_t, lower_episode[step].terminal = game:lower_step(lower_episode[step].a_t)
        ...
        if opt.model_target == 1 and not test_mode then
 		...
 		local comm = agent[i].lower_input[step][5]
		    ...
		    comm,
                ...
                -- Limit actions
                local action_range = game:getLowerActionRange(step, i, time_target)
                if action_range then
    ...
    upper_episode[1].r_t = game:upper_step()

    upper_episode.r = upper_episode[1].r_t

    upper_episode.steps = torch.ones(opt.bs)
...
function DRU(q_t, test_mode)
    ...
    local bound = opt.game_lower_action_space
    local q_t_n = q_t[{ {}, { 1, bound } }]:clone()
    local comm = q_t[{ {}, { bound + 1, opt.game_lower_action_space_total } }]:clone()
    if test_mode then
...
for e = 1, opt.nepisodes do
    ...
    for step = lower_episode.nsteps, 1, -1 do --iterates backwards through the steps
        ...
        stats.lower_td_comm[(e - 1) % opt.step + 1] = 0
        ...
        for i = 1, opt.game_nagents do
            ...
            for b = 1, opt.bs do
                ...
                if step <= lower_episode.steps[b] then
                    ...
                    if opt.model_dial == 0 then
                        if lower_episode[step].a_comm_t[b][i] > 0 then
                            if lower_episode[step].terminal[b] == 1 then
                                lower_td_comm_err[b] = lower_episode[step].r_t[b][i] - q_t[b][lower_episode[step].a_comm_t[b][i]]
                            else
                                local q_next_max
                                q_next_max = agent[i].q_comm_next_max[step + 1]:squeeze()
                                lower_td_comm_err[b] = lower_episode[step].r_t[b][i] + opt.gamma * q_next_max[b] - q_t[b][lower_episode[step].a_comm_t[b][i]]
                            end
                            lower_d_err[{ { b }, { lower_episode[step].a_comm_t[b][i] } }] = -lower_td_comm_err[b]
                        end
                    end
                    ...
                    if opt.model_dial == 1 and step < lower_episode.steps[b] then
                        -- Derivatives with respect to agent_i's message are stored in d_comm[b][i]
                        local bound = opt.game_lower_action_space
                        lower_d_err[{ { b }, { bound + 1, opt.game_lower_action_space_total } }]:add(lower_episode[step + 1].d_comm[b][i])
                    end
                end
            end
            ...
            if opt.model_dial == 0 then
                stats.lower_td_comm[(e - 1) % opt.step + 1] = stats.lower_td_comm[(e - 1) % opt.step + 1] + 0.5 * lower_td_comm_err:clone():pow(2):mean()
            end
            ...
            --'state' is the 5th input, so we can extract d_state
            agent[i].lower_d_state[step_back] = lower_grad[4]

            --For dial we need to write add the derivatives w/ respect to the incoming messages to the d_comm tracker
            if opt.model_dial == 1 then
                local comm_limited = game:getCommLimited(step, i)
                local comm_grad = lower_grad[5]

                if comm_limited then
                    for b = 1, opt.bs do
                        -- Agent could only receive the message if they were active
                        if comm_limited[b] ~= 0 then
                            lower_episode[step].d_comm[{ { b }, unpack(comm_limited[b]) }]:add(comm_grad[b])
                        end
                    end
                else
                    -- zero out own communication unless it's part of the switch riddle
                    comm_grad[{ {}, { i } }]:zero()
                    lower_episode[step].d_comm:add(comm_grad)
                end
            end
        end
    ...
    -- Upper Backward pass
        ...
        for i = 1, opt.game_nagents do
            ...
            -- Compute Q values
            local q_t = upperModel.agent[upperModel.id(step, i)].output
...
te_r = lower_episode.r:mean(1):mean(2):type(opt.dtype):squeeze()	
    ...
    if e == opt.step then
        ...
        stats.td_comm_avg = stats.lower_td_comm:mean()
    ...
    elseif e % opt.step == 0 then
        ...
        stats.td_comm_avg = stats.td_comm_avg * coef + stats.lower_td_comm:mean() * (1 - coef)
    ...
    if e % opt.step == 0 then
        log.infof('e=%d, td_err=%.3f, td_err_avg=%.3f, td_comm=%.3f, td_comm_avg=%.3f, up_tr_r=%.2f, up_tr_r_avg=%.2f, up_te_r=%.2f, up_te_r_avg=%.2f, low_tr_r=%.2f, low_tr_r_avg=%.2f, low_te_r=%.2f, low_te_r_avg=%.2f, st=%.1f, comm=%.1f%%, grad=%.3f, t/s=%.2f s, t=%d m',
            stats.e,
            stats.lower_td_err:mean(),
            stats.td_err_avg,
            stats.lower_td_comm:mean(),
            stats.td_comm_avg,
