...
cmd:option('-meta_eps', 0.05, 'epsilon-greedy policy')
...
cmd:option('-model_meta_rnn_size', 128, 'rnn size')
cmd:option('-model_meta_rnn_layers', 2, 'rnn layers')
...
cmd:option('-meta_model_dropout', 0, 'dropout')
...
cmd:option('-game_meta_action_space', 10, '')
...
if opt.model_rnn == 'lstm' then
    opt.model_meta_rnn_states = 2 * opt.model_meta_rnn_layers
...
elseif opt.model_rnn == 'gru' then
    opt.model_meta_rnn_states = opt.model_meta_rnn_layers
...
opt.game_meta_action_space_total = opt.game_meta_action_space
opt.game_lower_action_space_total = opt.game_lower_action_space

-- Initialise models
local metaModel = (require('model.' .. 'HierarchyMetaPlan'))(opt)
local upperModel = (require('model.' .. 'HierarchyCommPlan'))(opt)
...
metaModel.evaluate(metaModel.agent_target)
upperModel.evaluate(upperModel.agent_target)
lowerModel.evaluate(lowerModel.agent_target)

-- Get parameters
local meta_params, meta_gradParams, meta_params_target, _ = metaModel.getParameters()
...
print('initialised agent')
    ...
    -- Populate init state
    agent[i].meta_input = {}
    agent[i].meta_input_target = {}
    agent[i].meta_state = {}
    agent[i].meta_state_target = {}
    agent[i].meta_state[0] = {}
    agent[i].meta_state_target[0] = {}
    for j = 1, opt.model_meta_rnn_states do
        agent[i].meta_state[0][j] = torch.zeros(opt.bs, opt.model_meta_rnn_size):type(opt.dtype)
        agent[i].meta_state_target[0][j] = torch.zeros(opt.bs, opt.model_meta_rnn_size):type(opt.dtype)
    end

    agent[i].meta_d_state = {}
    agent[i].meta_d_state[0] = {}
    for j = 1, opt.model_meta_rnn_states do
        agent[i].meta_d_state[0][j] = torch.zeros(opt.bs, opt.model_meta_rnn_size):type(opt.dtype)
    end
...
local meta_d_err = torch.Tensor(opt.bs, opt.game_meta_action_space_total):type(opt.dtype)
local meta_td_err = torch.Tensor(opt.bs):type(opt.dtype)
...
local function run_episode(opt, game, metaModel, upperModel, lowerModel, agent, e, test_mode)
--print('run episode '.. e) 
    -- Test mode
...
--Run meta level for 1 step
        ...
        -- Forward pass
        meta_episode[1].a_t = torch.zeros(opt.bs, opt.game_nagents):type(opt.dtype)

        -- Iterate agents
        for i = 1, opt.game_nagents do
            agent[i].meta_input[step] = {
                meta_episode[1].s_t[i][{{},{1}}]:squeeze():type(opt.dtype),
                agent[i].id,
                agent[i].meta_state[step - 1]
            }


            -- Compute Q values
            local comm, state, q_t
            agent[i].meta_state[step], q_t = unpack(metaModel.agent[metaModel.id(step, i)]:forward(agent[i].meta_input[step]))
            ...
            if opt.model_dial == 0 then
                action_range, action_range_comm = game:getMetaActionRange(step, i)
            else
                action_range = game:getMetaActionRange(step, i)
            end

            -- If Limited action range
            if action_range then
                agent[i].meta_range = agent[i].meta_range or torch.range(1, opt.game_meta_action_space_total)
                ...
                for b = 1, opt.bs do
                    ...
                    if opt.model_dial == 0 then
                        ...
                        if action_range[b][2][1] > 0 then
                            ...
                            max_a[b] = agent[i].meta_range[{ action_range[b][2] }][a:squeeze()]
                        end
                        -- If comm action was taken
                        if action_range_comm[b][2][1] > 0 then
                            local v, a = torch.max(q_t[action_range_comm[b]], 2)
                            max_a_comm[b] = agent[i].meta_range[{ action_range_comm[b][2] }][a:squeeze()]
                        end
                    else
                        local v, a = torch.max(q_t[action_range[b]], 2)
                        max_a[b] = agent[i].meta_range[{ action_range[b][2] }][a:squeeze()]
                ...
                if not test_mode then
                    if opt.model_dial == 0 then
                        ...
                        if torch.uniform() < opt.upper_eps then
                            ...
                            if action_range then
                                if action_range[b][2][1] > 0 then
                                    local a_range = agent[i].meta_range[{ action_range[b][2] }]
                                    local a_idx = torch.random(a_range:nElement())
                                    meta_episode[1].a_t[b][i] = agent[i].meta_range[{ action_range[b][2] }][a_idx]
                                end
                            else
                                meta_episode[1].a_t[b][i] = torch.random(opt.game_upper_action_space)
                            end
                        end
                    else
                        if torch.uniform() < opt.upper_eps then
                            if action_range then
                                ...
                                meta_episode[1].a_t[b][i] = torch.random(q_t[b]:size(1))
                            end
                        end
                    end
                end
    ...
    meta_episode[1].r_t=torch.zeros(opt.bs, opt.game_nagents)

    comm_max = meta_episode[1].a_t:clone()




    --Run upper level for by meta defined steps
    local steps = torch.max(comm_max)
    while step <= steps do
		...
		if test_mode then
            ...
            local max_value, max_a, max_a_comm
            if opt.model_dial == 0 then
                action_range, action_range_comm = game:getUpperActionRange(step, i, comm_max)
            else
                action_range = game:getUpperActionRange(step, i, comm_max)
            end
		...
		if step == torch.max(comm_max[b]) then
		    upper_episode[step].terminal[b] = 1
		end
        ...
        -- Forward next step
	upper_episode[step].r_t=torch.zeros(opt.bs, opt.game_nagents)
        step = step + 1
        upper_episode[step].s_t = game:getState()
	upper_episode[step].terminal = upper_episode[step-1].terminal:clone()
    ...
    time_target = torch.zeros(opt.bs,opt.game_nagents):type(opt.dtype)

    for time = 1, steps do

	actions = upper_episode[time].a_t:clone()

        for b = 1, opt.bs do

	    for agent = 1, opt.game_nagents do  

	        if time == comm_max[b][agent] then
		    time_target[b][agent] = actions[b][agent]
	        end
	    end
        end
    end
...
--[[if steps ==3 then
print(torch.cat(comm_max,upper_episode[1].a_t,2):cat(upper_episode[2].a_t,2):cat(upper_episode[3].a_t,2):cat(time_target,2))
elseif steps >3 then
print(torch.cat(comm_max,upper_episode[1].a_t,2):cat(upper_episode[2].a_t,2):cat(upper_episode[3].a_t,2):cat(upper_episode[4].a_t,2):cat(time_target,2))
end--]]
    ...
    meta_episode[1].r_t = game:upper_step()
    ...
    for time = 1,steps do
	    ...
	    for agent = 1, opt.game_nagents do  
		if time == comm_max[b][agent] then
  	            upper_episode[time].r_t[b][agent] = game:upper_step()[b][agent]
    ...
    meta_episode.r = meta_episode[1].r_t
    upper_episode.r = game:upper_step()
    ...
    return meta_episode, upper_episode, lower_episode, agent
end
...
for e = 1, opt.nepisodes do
    ...
    -- Model training
    metaModel.training(metaModel.agent)
    ...
    -- Run episode
    meta_episode , upper_episode , lower_episode , agent = run_episode(opt, game, metaModel, upperModel, lowerModel, agent, e)
...
-- Meta Backward pass
    local step=1
        ...
        for i = 1, opt.game_nagents do
            ...
            -- Compute Q values
            local state, q_t = unpack(metaModel.agent[metaModel.id(step, i)].output)
            ...
            for b = 1, opt.bs do
                ...
                for j = 1, opt.model_meta_rnn_states do
                    agent[i].meta_d_state[0][j][b]:zero()
                end
                

                if meta_episode[step].a_t[b][i] > 0 then
                    meta_td_err[b] = meta_episode[step].r_t[b][i] - q_t[b][meta_episode[step].a_t[b][i]]
                    meta_d_err[{ { b }, { meta_episode[step].a_t[b][i] } }] = -meta_td_err[b]
                else
                    error('Error!')
                end
            ...
            -- Backward pass
            local meta_grad = metaModel.agent[metaModel.id(step, i)]:backward(agent[i].meta_input[step], {
                agent[i].meta_d_state[0],
                meta_d_err
            })

            --'state' is the 3rd input, so we can extract d_state
            agent[i].meta_d_state[1] = meta_grad[3]

           
        end

    -- Update gradients
    local meta_feval = function(x)

        -- Normalise Gradients
        meta_gradParams:div(opt.game_nagents * opt.bs)

        -- Clip Gradients
        meta_gradParams:clamp(-10, 10)

        return nil, meta_gradParams
    end
...
te_r = lower_episode.r:mean(1):mean(2):type(opt.dtype):squeeze()	
	    ...
	    upper_learningrate = opt.learningrate 
            meta_learningrate = opt.learningrate 
print('reached 100')
	    ...
	    upper_learningrate = opt.learningrate *0.1
            meta_learningrate = opt.learningrate * 0.1
print('reached 96')
	    ...
	    upper_learningrate = opt.learningrate *0.01
            meta_learningrate = opt.learningrate * 0.01
print('reached 92')
	    ...
	    upper_learningrate = opt.learningrate *0.001
            meta_learningrate = opt.learningrate * 0.001
print('reached 85')
	    ...
	    upper_learningrate = opt.learningrate *0.0001
            meta_learningrate = opt.learningrate * 0.0001
print('reached 75')
	    ...
	    upper_learningrate = opt.learningrate * 0.00001
            meta_learningrate = opt.learningrate * 0.00001
	end

    meta_optim_config.learningRate = meta_learningrate
    meta_optim_func(meta_feval, meta_params, meta_optim_config, optim_state)
    ...
    if e % opt.step == 0 then
        stats.meta_grad_norm = meta_gradParams:norm() / meta_gradParams:nElement() * 1000
    ...
    if e % opt.step_test == 0 then
        ...
        local meta_episode, upper_episode , lower_episode , _ = run_episode(opt, game, metaModel, upperModel, lowerModel, agent, e, true)
