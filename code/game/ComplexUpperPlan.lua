...
local ComplexUpperPlan = class('ComplexUpperPlan')
...
function ComplexUpperPlan:__init(opt)
...
function ComplexUpperPlan:reset(episode)
    ...
    -- Whos ComplexUpperPlan is at which position? 
...
function ComplexUpperPlan:getActionRange(step, agent)
    local range = {}
    if self.opt.model_dial == 1 then           
        ...
        return range
    else					--the rial option was not updated to fit the ComplexUpperPlan game yet
        local comm_range = {}
        for b = 1, self.opt.bs do
            if self.pulled_lever[b][agent] == 0 then
...
function ComplexUpperPlan:getCommLimited(step, i)
...
function ComplexUpperPlan:getReward(a_t,episode)
...
function ComplexUpperPlan:step(a_t,episode)
...
function ComplexUpperPlan:getState()
    local state = {}
...
function ComplexUpperPlan:imitateAction()
