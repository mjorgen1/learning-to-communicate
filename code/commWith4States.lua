    ...
    while step <= steps and episode.ended:sum() < opt.bs do
        ...
        for i = 1, opt.game_nagents do
            agent[i].input[step] = {
                ...
                episode[step].s_t[i][{{},{3}}]:squeeze():type(opt.dtype),
                episode[step].s_t[i][{{},{4}}]:squeeze():type(opt.dtype),
                agent[i].id,
	...
	if test_mode then
	   ...
	   -- print('terminated: '.. episode[step].terminal[1])
        ...
        if opt.model_target == 1 and not test_mode then
            for i = 1, opt.game_nagents do
                local comm = agent[i].input[step][7]
