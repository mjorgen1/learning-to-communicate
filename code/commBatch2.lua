...
local function run_episode(opt, game, model, agent, e, test_mode)
        ...
        for i = 1, opt.game_nagents do
            ...
            if opt.game_comm_bits > 0 and opt.game_nagents > 1 then
                ...
                if comm_limited then
                    ...
                    --inside changed for copying everything from b1 to b2
                    for b = 1, opt.bs do  
                        if comm_limited[1] == 0 then                  
                            comm_lim[{ { b } }]:zero()
                        else
                            comm_lim[{ { b } }] = comm[{ { 1 }, unpack(comm_limited[1]) }]
                        end
                    end
            ...
            if opt.model_action_aware == 1 then
                ...
                if opt.model_dial == 0 then
                    ...
                    if step > 1 then
                        for b = 1, opt.bs do  --inside changed for copying everything from b1 to b2
                            -- Last action
                            if episode[step - 1].a_t[1][i] > 0 then                  
                                la[1][{ { b } }] = episode[step - 1].a_t[1][i] + 1
                            end
                            -- Last comm action                                       
                            if episode[step - 1].a_comm_t[1][i] > 0 then
                                la[2][{ { b } }] = episode[step - 1].a_comm_t[1][i] - opt.game_action_space + 1
                            end
                        end
                    end
            ...
            for b = 1, opt.bs do  
                ...
                if not test_mode and not im_learning then
                    if opt.model_dial == 0 then
                        ...
                        if torch.uniform() < opt.eps then
                            if action_range then
                                if action_range[1][2][1] > 0 then
                                    local a_range = agent[i].range[{ action_range[1][2] }]
                                    local a_idx = torch.random(a_range:nElement())
                                    episode[step].a_t[b][i] = agent[i].range[{ action_range[1][2] }][a_idx]
                                end
                            else
                        ...
                        if opt.game_comm_bits > 0 and torch.uniform() < opt.eps then
                            if action_range then
                                if action_range_comm[1][2][1] > 0 then  
                                    local a_range = agent[i].range[{ action_range_comm[1][2] }]
                                    local a_idx = torch.random(a_range:nElement())
                                    episode[step].a_comm_t[b][i] = agent[i].range[{ action_range_comm[1][2] }][a_idx]
                                end
                            else
                ...
                if not test_mode and im_learning then
                    ...
                    if(opt.model_dial == 0) then
                        episode[step].a_comm_t[b][i] = comm_actions[1][i]
                    end
                ...
                if step <= opt.nsteps then
                    ...
                    if opt.model_dial == 1 then
                        episode[step + 1].comm[b][i] = comm[1]
                    else
                        local a_t = episode[step].a_comm_t[1][i] - opt.game_action_space
                        if a_t > 0 then
            ...
            --to make sure after epsilon greedy the actions are still the same
            episode[step].a_t[2] = episode[step].a_t[1]
        end
	...
	if test_mode then
        ...
        print(episode[step].r_t[2])
        print('terminated: '.. episode[step].terminal[1])
        print(episode[step].terminal[2])--]]
	end
        ...
        if step <= opt.nsteps then
            ...
            for b = 1, opt.bs do
                if episode.ended[b] == 0 then
                    ...
                    episode.r[{ { b } }]:add(episode[step].r_t[1])
                    -- Check if terminal
            ...
            for i = 1, opt.game_nagents do
                ...
                if opt.model_dial == 0 and opt.game_comm_bits > 0 then
                    ...
                    if action_range then
                        ...
                        for b = 1, opt.bs do
                            if action_range[1][2][1] > 0 then
                                agent[i].q_next_max[step][b], _ = torch.max(q_t_target[action_range[1]], 2)
                            else
                            ...
                            if action_range_comm[1][2][1] > 0 then
                                agent[i].q_comm_next_max[step][b], _ = torch.max(q_t_target[action_range_comm[1]], 2)
                            else
                                agent[i].q_comm_next_max[step][b], _ = torch.max(q_t_target[action_range[1]], 2)
                            end
                        end
                    else
                ...
                if opt.model_dial == 1 then
                    --inside changed for copying everything from b1 to b2
                    for b = 1, opt.bs do
                        episode[step + 1].comm_target[b][i] = comm[1]
                    end
                end
            end
        end
...
print('zeroed')
			    ...
			    stats.q_err = td_err[1]
                            ...
                            td_err[b] = episode[step].r_t[1][i] + opt.gamma * q_next_max[1] - q_t[1][episode[step].a_t[1][i]]
                        end
                        d_err[{ { b }, { episode[step].a_t[b][i] } }] = -td_err[1]

                        --preparation for the case tables
                        if ((step == episode.steps[1]) and (b == 1) and (i == 1)) then
			    ...
			    --print(episode[step].r_t[1][i])
                    ...
                    if opt.model_dial == 0 then
                        if episode[step].a_comm_t[1][i] > 0 then
                            if episode[step].terminal[1] == 1 then
                                td_comm_err[b] = episode[step].r_t[1][i] - q_t[1][episode[step].a_comm_t[1][i]]
                            else
                                local q_next_max
                                if opt.model_avg_q == 1 and episode[step].a_t[1][i] > 0 then
                                    q_next_max = (agent[i].q_next_max[step + 1]:squeeze() + agent[i].q_comm_next_max[step + 1]:squeeze()) / 2
                                else
                                    q_next_max = agent[i].q_comm_next_max[step + 1]:squeeze()
                                end
                                td_comm_err[b] = episode[step].r_t[1][i] + opt.gamma * q_next_max[b] - q_t[1][episode[step].a_comm_t[1][i]]
                            end
                            d_err[{ { b }, { episode[step].a_comm_t[b][i] } }] = -td_comm_err[1]
                        end
                    end
                    ...
                    if opt.model_dial == 1 and step < episode.steps[1] then
...
--print(comm_grad)
                ...
                if comm_limited then
                    for b = 1, opt.bs do  --changed for the sake of copying everything from batch 1 to b2
                        ...
                        if comm_limited[1] ~= 0 then
                            episode[step].d_comm[{ { b }, unpack(comm_limited[1]) }]:add(comm_grad[1])
                            if b == 1 and i == 1 then
                                stats.comm_gradient = comm_grad[1]
                                --print(stats.comm_gradient)
                                --print(stats.comm_gradient[1][1])
                                --print(stats.comm_gradient[1][2])
                            end
                        end
                    end
                else
