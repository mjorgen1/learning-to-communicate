...
return function(opt)
    ...
    local function create_agent()
        ...
        -- if opt.model_bn == 1 then model_input:add(nn.BatchNormalization(opt.model_lower_rnn_size)) end
	...
	local in_state_2 = nn.Identity()()
        ...
        if opt.game_comm_bits > 0 and opt.game_nagents > 1 then
            ...
            model_comm:add(nn.Linear(comm_size, opt.model_lower_rnn_size))
            if opt.model_comm_narrow == 1 then
        ...
        if opt.model_action_aware == 1 then
            ...
            if opt.model_dial == 0 then
                local in_action_aware = nn.CAddTable(2)({
                    nn.LookupTable(opt.game_lower_action_space + 1, opt.model_lower_rnn_size)(nn.SelectTable(1)(in_action)),
                    nn.LookupTable(opt.game_comm_bits + 1, opt.model_lower_rnn_size)(nn.SelectTable(2)(in_action))
                })
