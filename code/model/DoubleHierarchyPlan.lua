...
return function(opt)
    ...
    function exp.save(opt, stats, model)
        if stats.e % opt.step == 0 then
            ...
            if stats.e % (opt.step * 10) == 0 then
                ...
                -- save model
                local filename = paths.concat(exp.save_path, 'meta_exp.t7')
                torch.save(filename, { opt, stats, model.meta_agent })
    ...
    function exp.getParameters()
        ...
        local aa = nn.Container()
        for i = 1, #exp.meta_agent do
            aa:add(exp.meta_agent[i])
        end
        local meta_params, meta_gradParams = aa:getParameters()
        ...
        log.infof('Creating model(s), params=%d', meta_params:nElement()+upper_params:nElement()+lower_params:nElement())
        ...
        local cc = nn.Container()
        for i = 1, #exp.meta_agent_target do
            cc:add(exp.meta_agent_target[i])
        end
        local meta_params_target, meta_gradParams_target = cc:getParameters()

        log.infof('Creating target model(s), params=%d', lower_params_target:nElement())

        return meta_params, meta_gradParams, upper_params, upper_gradParams, lower_params, lower_gradParams ,meta_params_target, meta_gradParams_target, lower_params_target, lower_gradParams_target
    end
    ...
    local function create_meta_agent()
        ...
        model_state:add(nn.LookupTable(opt.nsteps+1, opt.model_meta_rnn_size))

        -- RNN
        local model_rnn
        if opt.model_rnn == 'lstm' then
            model_rnn = LSTM(opt.model_meta_rnn_size,
                opt.model_meta_rnn_size,
                opt.model_meta_rnn_layers,
                opt.meta_model_dropout,
                opt.model_bn == 1)
        elseif opt.model_rnn == 'gru' then
            model_rnn = GRU(opt.model_meta_rnn_size,
                opt.model_meta_rnn_size,
                opt.model_meta_rnn_layers,
                opt.meta_model_dropout)
        end
        ...
        if opt.meta_model_dropout > 0 then model_out:add(nn.Dropout(opt.meta_model_dropout)) end
        model_out:add(nn.Linear(opt.model_meta_rnn_size, opt.model_meta_rnn_size))
        model_out:add(nn.ReLU(true))
        model_out:add(nn.Linear(opt.model_meta_rnn_size, opt.game_meta_action_space_total))
        ...
        local in_all = {
            ...
            nn.LookupTable(opt.game_nagents, opt.model_meta_rnn_size)(in_id)
        }
        ...
        -- Restore state
        for i = 1, opt.model_meta_rnn_states do
        ...
        local rnn_out = nn.SelectTable(opt.model_meta_rnn_states)(rnn_output)
    ...
    -- Create model
    local meta_agent = create_meta_agent()
    local meta_agent_target = meta_agent:clone()
    ...
    if opt.model_know_share == 1 then
        exp.meta_agent = util.cloneManyTimes(meta_agent, opt.game_nagents * (opt.nsteps + 1))
        exp.meta_agent_target = util.cloneManyTimes(meta_agent_target, opt.game_nagents * (opt.nsteps + 1))
        ...
        exp.meta_agent = {}
        exp.meta_agent_target = {}

        local agent_copies = util.copyManyTimes(meta_agent, opt.game_nagents)
        local agent_target_copies = util.copyManyTimes(meta_agent_target, opt.game_nagents)
        ...
        for i = 1, opt.game_nagents do
            ...
            for s = 1, opt.nsteps + 1 do
                exp.meta_agent[exp.id(s, i)] = unrolled[s]
                exp.meta_agent_target[exp.id(s, i)] = unrolled_target[s]
            end
        end
